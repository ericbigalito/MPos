...
class Sale extends CI_Model
{
	...
	public function get_info_invoicing($sale_id)
	{
	...
	function get_invoicing_totaled_by_id($shift_start, $shift_end)
	{
		$employee_id=$this->Employee->get_logged_in_employee_info()->person_id;
		$this->db->select('invoicing.sale_id', false);
		$this->db->from('invoicing');
		$this->db->join('invoicing_payments','invoicing_payments.sale_id=invoicing.sale_id');
		$this->db->where('invoicing_payments.payment_date >=', $shift_start);
		$this->db->where('invoicing_payments.payment_date <=', $shift_end);
		$this->db->where('employee_id', $employee_id);
		$this->db->where($this->db->dbprefix('invoicing').'.deleted', 0);
		...
		if (count($sale_ids) > 0)
		{
			$where = 'WHERE '.$this->db->dbprefix('invoicing').'.sale_id IN('.implode(',',$sale_ids).')';
			$this->_create_invoicing_items_temp_table_query($where);
			$this->db->select('sale_id, SUM(total) as total', false);
			$this->db->from('invoicing_items_temp');
			$this->db->group_by('sale_id');
    ...
    /* updating invoice page */
    function update_invoicing($sale_data, $sale_id, $invoice_check)
    {
        $this->db->where('sale_id', $sale_id);
        $success = $this->db->update('invoicing',$sale_data);

        return $success;

    }
	
	function save ($items,$customer_id,$employee_id,$comment,$show_comment_on_receipt,$payments,$sale_id=false, $suspended = 0, $cc_ref_no = '', $auth_code = '', $change_sale_date=false,$balance=0, $store_account_payment = 0,$invoice_check,$invoice_check2)
	{
		if(count($items)==0)
			return -1;
		...
		if($sale_id)
		{
            if($invoice_check != 1){
                if($invoice_check2 == 1){
                    /*$old_date=$this->get_info_invoicing($sale_id)->row_array();
		...
		if ($sale_id)
		{
			if($invoice_check != 1){
                ...
                if($invoice_check2 == 1){
				/*	//getting the invoice amount first;					
					$this->db->select('invoicing_payments.payment_amount', false);
					$this->db->from('invoicing_payments');
					$this->db->where('sale_id', $sale_id);
					$invoice_frm_db = $this->db->get();

					// get the total amount from the sale
					$invoice_amt = $this->sale_lib->get_total();

					//getting the differenceof actual invoice and paid
					if($invoice_frm_db - $invoice_amt){
						$diff_inv = $invoice_frm_db - $invoice_amt;
					} */
					
                    //transfering invoice to sales and deleting invoice from invoice data
					
                    $this->delete_invoicing($sale_id, true);
			...
			if($invoice_check == 1){
				...
				$this->db->where('sale_id', $sale_id);
				$this->db->update('invoicing', $sales_data);
			}
		...
		else
		{
			...
			if($invoice_check == 1){
				$this->db->insert('invoicing',$sales_data);
				$sale_id = $this->db->insert_id();
			}
		...
		foreach($payments as $payment_id=>$payment)
		{
			...
			if($invoice_check == 1){
				$this->db->insert('invoicing_payments',$sales_payments_data);
			}
		...
		foreach($items as $line=>$item)
		{
			if (isset($item['item_id']))
			{
				...
				if($invoice_check == 1){
					$this->db->insert('invoicing_items',$sales_items_data);
				}
				...
				{			
					...
					$this->email->from($this->Location->get_info_for_key('email') ? $this->Location->get_info_for_key('email') : 'no-reply@tracecoreafrica.com', $this->config->item('company'));
				...
				if (!$cur_item_info->is_service)
				{
					...
					(
						...
						'quantity_left'=>$cur_item_location_info->quantity,
						'location_id' => $this->Employee->get_logged_in_employee_current_location_id() 
					);
			...
			else
			{
				...
				if($invoice_check == 1){
					$this->db->insert('invoicing_item_kits',$sales_item_kits_data);
				}
 			...
 			if ($customer_id == -1 or $customer->taxable)
 			{
				if (isset($item['item_id']))
				{
					foreach($this->Item_taxes_finder->get_info($item['item_id']) as $row)
					{
						...
						if($invoice_check == 1){
							$this->db->insert('invoicing_items_taxes', array(
								'sale_id' 	=>$sale_id,
								'item_id' 	=>$item['item_id'],
								'line'      =>$item['line'],
								'name'		=>$row['name'],
				...
				else
				{
					foreach($this->Item_kit_taxes_finder->get_info($item['item_kit_id']) as $row)
					{
						...
						if($invoice_check == 1){
							$this->db->insert('invoicing_item_kits_taxes', array(
								'sale_id' 		=>$sale_id,
								'item_kit_id'	=>$item['item_kit_id'],
								'line'      	=>$item['line'],
								'name'			=>$row['name'],
    ...
    function delete_invoicing($sale_id, $all_data = false)
    {
        $employee_id=$this->Employee->get_logged_in_employee_info()->person_id;

        $this->db->select('invoicing.location_id, item_id, quantity_purchased');
        $this->db->from('invoicing_items');
        $this->db->join('invoicing', 'invoicing.sale_id = invoicing_items.sale_id');
        $this->db->where('invoicing_items.sale_id', $sale_id);
        ...
        $this->db->select('invoicing.location_id, item_kit_id, quantity_purchased');
        $this->db->from('invoicing_item_kits');
        $this->db->join('invoicing', 'invoicing.sale_id = invoicing_item_kits.sale_id');
        $this->db->where('invoicing_item_kits.sale_id', $sale_id);
        ...
        if ($all_data)
        {
            //Run these queries as a transaction, we want to make sure we do all or nothing
            $this->db->trans_start();
            $this->db->delete('invoicing_payments', array('sale_id' => $sale_id));
            $this->db->delete('invoicing_items_taxes', array('sale_id' => $sale_id));
            $this->db->delete('invoicing_items', array('sale_id' => $sale_id));
            $this->db->delete('invoicing_item_kits_taxes', array('sale_id' => $sale_id));
            $this->db->delete('invoicing_item_kits', array('sale_id' => $sale_id));
            $this->db->trans_complete();
        }

        $this->db->where('sale_id', $sale_id);
        return $this->db->update('invoicing', array('deleted' => 1,'deleted_by'=>$employee_id));
    }
    ...
    function undelete_invoicing($sale_id)
    {
        $employee_id=$this->Employee->get_logged_in_employee_info()->person_id;
        ...
        $this->db->where('sale_id', $sale_id);
        return $this->db->update('invoicing', array('deleted' => 0, 'deleted_by' => NULL));
    }
    ...
    function get_invoicing_items($sale_id)
    {
        $this->db->from('invoicing_items');
        $this->db->where('sale_id',$sale_id);
        return $this->db->get();
    ...
    function get_invoicing_item_kits($sale_id)
    {
        $this->db->from('invoicing_item_kits');
        $this->db->where('sale_id',$sale_id);
        return $this->db->get();
	...
	//getting invoicing data
	function get_invoicing_payments($sale_id)
	{
		$this->db->from('invoicing_payments');
		$this->db->where('sale_id',$sale_id);
		return $this->db->get();
	...
	/*... getting invoice comments*/
	function get_comment_invoicing($sale_id)
	{
		$this->db->from('invoicing');
		$this->db->where('sale_id',$sale_id);
		return $this->db->get()->row()->comment;
	...
	//geeting invoice comments for reciepts
	function get_comment_on_receipt_invoicing($sale_id)
	{
		$this->db->from('invoicing');
		$this->db->where('sale_id',$sale_id);
		return $this->db->get()->row()->show_comment_on_receipt;
	...
	//We create a temp table that allows us to do easy report/sales queries
	public function create_invoicing_items_temp_table($params)
	{
		if (!$this->session->userdata('sales_report_running'))
		{
			$this->session->set_userdata('sales_report_running', TRUE);
			...
			if (isset($params['start_date']) && isset($params['end_date']))
			{
				$where = 'WHERE sale_time BETWEEN "'.$params['start_date'].'" and "'.$params['end_date'].'"'.' and '.$this->db->dbprefix('invoicing').'.location_id='.$this->db->escape($location_id). (($this->config->item('hide_store_account_payments_in_reports') ) ? ' and '.$this->db->dbprefix('invoicing').'.store_account_payment=0' : '');
			...
			elseif ($this->config->item('hide_suspended_sales_in_reports'))
			{
				$where .='WHERE suspended = 0'.' and '.$this->db->dbprefix('invoicing').'.location_id='.$this->db->escape($location_id).(($this->config->item('hide_store_account_payments_in_reports') ) ? ' and '.$this->db->dbprefix('invoicing').'.store_account_payment=0' : '');
			}

			if ($where == '')
			{
				$where = 'WHERE '.$this->db->dbprefix('invoicing').'.location_id='.$this->db->escape($location_id).(($this->config->item('hide_store_account_payments_in_reports') ) ? ' and '.$this->db->dbprefix('invoicing').'.store_account_payment=0' : '');
			}
	...
	function _create_invoicing_items_temp_table_query($where)
	{
		set_time_limit(0);

		$this->db->query("CREATE TEMPORARY TABLE ".$this->db->dbprefix('invoicing_items_temp')."
		(SELECT ".$this->db->dbprefix('invoicing').".deleted as deleted,".$this->db->dbprefix('invoicing').".deleted_by as deleted_by, sale_time, date(sale_time) as sale_date, ".$this->db->dbprefix('invoicing_items').".sale_id, comment,payment_type, customer_id, employee_id, 
		".$this->db->dbprefix('items').".item_id, NULL as item_kit_id, supplier_id, quantity_purchased, item_cost_price, item_unit_price, category, 
		discount_percent, (item_unit_price*quantity_purchased-item_unit_price*quantity_purchased*discount_percent/100) as subtotal,
		".$this->db->dbprefix('invoicing_items').".line as line, serialnumber, ".$this->db->dbprefix('invoicing_items').".description as description,
		(item_unit_price*quantity_purchased-item_unit_price*quantity_purchased*discount_percent/100)+(item_unit_price*quantity_purchased-item_unit_price*quantity_purchased*discount_percent/100)*(SUM(CASE WHEN cumulative != 1 THEN percent ELSE 0 END)/100) 
		+(((item_unit_price*quantity_purchased-item_unit_price*quantity_purchased*discount_percent/100)*(SUM(CASE WHEN cumulative != 1 THEN percent ELSE 0 END)/100) + (item_unit_price*quantity_purchased-item_unit_price*quantity_purchased*discount_percent/100))
		*(SUM(CASE WHEN cumulative = 1 THEN percent ELSE 0 END))/100) as total,
		(item_unit_price*quantity_purchased-item_unit_price*quantity_purchased*discount_percent/100)*(SUM(CASE WHEN cumulative != 1 THEN percent ELSE 0 END)/100) 
		...
		*(SUM(CASE WHEN cumulative = 1 THEN percent ELSE 0 END))/100) as tax,
		(item_unit_price*quantity_purchased-item_unit_price*quantity_purchased*discount_percent/100) - (item_cost_price*quantity_purchased) as profit
		FROM ".$this->db->dbprefix('invoicing_items')."
		INNER JOIN ".$this->db->dbprefix('invoicing')." ON  ".$this->db->dbprefix('invoicing_items').'.sale_id='.$this->db->dbprefix('invoicing').'.sale_id'."
		INNER JOIN ".$this->db->dbprefix('items')." ON  ".$this->db->dbprefix('invoicing_items').'.item_id='.$this->db->dbprefix('items').'.item_id'."
		LEFT OUTER JOIN ".$this->db->dbprefix('suppliers')." ON  ".$this->db->dbprefix('items').'.supplier_id='.$this->db->dbprefix('suppliers').'.person_id'."
		LEFT OUTER JOIN ".$this->db->dbprefix('invoicing_items_taxes')." ON  "
			.$this->db->dbprefix('invoicing_items').'.sale_id='.$this->db->dbprefix('invoicing_items_taxes').'.sale_id'." and "
			.$this->db->dbprefix('invoicing_items').'.item_id='.$this->db->dbprefix('invoicing_items_taxes').'.item_id'." and "
			.$this->db->dbprefix('invoicing_items').'.line='.$this->db->dbprefix('invoicing_items_taxes').'.line'. "
		$where
		GROUP BY sale_id, item_id, line) 
		UNION ALL
		(SELECT ".$this->db->dbprefix('invoicing').".deleted as deleted,".$this->db->dbprefix('invoicing').".deleted_by as deleted_by, sale_time, date(sale_time) as sale_date, ".$this->db->dbprefix('invoicing_item_kits').".sale_id, comment,payment_type, customer_id, employee_id, 
		NULL as item_id, ".$this->db->dbprefix('item_kits').".item_kit_id, '' as supplier_id, quantity_purchased, item_kit_cost_price, item_kit_unit_price, category, 
		discount_percent, (item_kit_unit_price*quantity_purchased-item_kit_unit_price*quantity_purchased*discount_percent/100) as subtotal,
		".$this->db->dbprefix('invoicing_item_kits').".line as line, '' as serialnumber, ".$this->db->dbprefix('invoicing_item_kits').".description as description,
		(item_kit_unit_price*quantity_purchased-item_kit_unit_price*quantity_purchased*discount_percent/100)+(item_kit_unit_price*quantity_purchased-item_kit_unit_price*quantity_purchased*discount_percent/100)*(SUM(CASE WHEN cumulative != 1 THEN percent ELSE 0 END)/100) 
		+(((item_kit_unit_price*quantity_purchased-item_kit_unit_price*quantity_purchased*discount_percent/100)*(SUM(CASE WHEN cumulative != 1 THEN percent ELSE 0 END)/100) + (item_kit_unit_price*quantity_purchased-item_kit_unit_price*quantity_purchased*discount_percent/100))
		*(SUM(CASE WHEN cumulative = 1 THEN percent ELSE 0 END))/100) as total,
		(item_kit_unit_price*quantity_purchased-item_kit_unit_price*quantity_purchased*discount_percent/100)*(SUM(CASE WHEN cumulative != 1 THEN percent ELSE 0 END)/100) 
		...
		*(SUM(CASE WHEN cumulative = 1 THEN percent ELSE 0 END))/100) as tax,
		(item_kit_unit_price*quantity_purchased-item_kit_unit_price*quantity_purchased*discount_percent/100) - (item_kit_cost_price*quantity_purchased) as profit
		FROM ".$this->db->dbprefix('invoicing_item_kits')."
		INNER JOIN ".$this->db->dbprefix('invoicing')." ON  ".$this->db->dbprefix('invoicing_item_kits').'.sale_id='.$this->db->dbprefix('invoicing').'.sale_id'."
		INNER JOIN ".$this->db->dbprefix('item_kits')." ON  ".$this->db->dbprefix('invoicing_item_kits').'.item_kit_id='.$this->db->dbprefix('item_kits').'.item_kit_id'."
		LEFT OUTER JOIN ".$this->db->dbprefix('invoicing_item_kits_taxes')." ON  "
			.$this->db->dbprefix('invoicing_item_kits').'.sale_id='.$this->db->dbprefix('invoicing_item_kits_taxes').'.sale_id'." and "
			.$this->db->dbprefix('invoicing_item_kits').'.item_kit_id='.$this->db->dbprefix('invoicing_item_kits_taxes').'.item_kit_id'." and "
			.$this->db->dbprefix('invoicing_item_kits').'.line='.$this->db->dbprefix('invoicing_item_kits_taxes').'.line'. "
		$where
		GROUP BY sale_id, item_kit_id, line) ORDER BY sale_id, line");
	}
	...
	function get_recent_invoicing_for_customer($customer_id)
	{
		$return = array();

		if (!$this->create_invoicing_items_temp_table(array('start_date' =>date('Y-m-d', strtotime('-60 days')), 'end_date' => date('Y-m-d').' 23:59:59')))
		{
			return array();
		}
