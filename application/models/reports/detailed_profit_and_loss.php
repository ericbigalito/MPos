...
class Detailed_profit_and_loss extends Report
{
	...
	public function getData()
	{
		...
		//getting paid store account ammount

		$sales_totals_paid = array();
		$this->db->select('sale_id, sum(total) as total', false);
		$this->db->from('sales_items_temp');
		$this->db->group_by('sale_id');
			
		foreach($this->db->get()->result_array() as $sale_total_row_paid)
		{
			$sales_totals_paid[$sale_total_row_paid['sale_id']] = $sale_total_row_paid['total'];
		}
		...
		$this->db->order_by('sale_id, payment_type');
		$sales_payments_paid = $this->db->get()->result_array();
		
		$payments_by_sale_paid = array();
		
		foreach($sales_payments_paid as $row)
		{
        	$payments_by_sale_paid[$row['sale_id']][] = $row;
		}
		
		$payment_data_paid = array();
		
		foreach($payments_by_sale_paid as $sale_id => $payment_rows)
		{
			if(isset($sales_totals_paid[$sale_id])){
				$total_sale_balance_paid = $sales_totals_paid[$sale_id];
			}
			
			foreach($payment_rows as $row)
			{
				$payment_amount_paid = $row['payment_amount'] <= $total_sale_balance_paid ? $row['payment_amount'] : $total_sale_balance_paid;
				
				if (!isset($payment_data_paid[$row['payment_type']]))
				{
					$payment_data_paid[$row['payment_type']] = array('payment_type' => $row['payment_type'], 'payment_amount' => 0 );
				}
				
				if ($total_sale_balance_paid != 0)
				{
					$payment_data_paid[$row['payment_type']]['payment_amount'] += $payment_amount_paid;
				}
				
				$total_sale_balance_paid-=$payment_amount_paid;
			}
		}
				
		$data['sales_by_payments_paid'] = $payment_data_paid;
		
		
		foreach($data['sales_by_payments_paid'] as $payment_data_paid)
		{
			$total+=$payment_data_paid['payment_amount'];
		}

		//end of getting paid store account
