...
class Invoicing_generator extends Report
{
	...
	public function getData()
	{
		...
		if ($this->params['matched_items_only'])
		{
			...
			$this->db->join('people as employee', 'invoicing_items_temp.employee_id = employee.person_id');
			$this->db->join('people as customer', 'invoicing_items_temp.customer_id = customer.person_id', 'left');
			$this->_searchSalesQueryParams();
			$this->db->where('invoicing_items_temp.deleted', 0);
			...
			$this->db->select('sale_id, items.item_id, item_kits.item_kit_id, sale_time, sale_date, item_number, items.product_id as item_product_id,item_kits.product_id as item_kit_product_id, item_kit_number, items.name as item_name, item_kits.name as item_kit_name, invoicing_items_temp.category, quantity_purchased, quantity_purchased as items_purchased, serialnumber, invoicing_items_temp.description, subtotal,total, tax, profit, discount_percent', false);
			$this->db->from('invoicing_items_temp');
			$this->db->join('items', 'invoicing_items_temp.item_id = items.item_id', 'left');
			$this->db->join('item_kits', 'invoicing_items_temp.item_kit_id = item_kits.item_kit_id', 'left');
			$this->_searchSalesQueryParams();		
			if (!empty($sale_ids))
			{
		...
		else
		{
			...
			$this->db->select('sale_id, items.item_id, item_kits.item_kit_id, sale_time, sale_date, item_number, items.product_id as item_product_id,item_kits.product_id as item_kit_product_id, item_kit_number, items.name as item_name, item_kits.name as item_kit_name, invoicing_items_temp.category, quantity_purchased, serialnumber, invoicing_items_temp.description, subtotal,total, tax, profit, discount_percent', false);
			$this->db->from('invoicing_items_temp');
			$this->db->join('items', 'invoicing_items_temp.item_id = items.item_id', 'left');
			$this->db->join('item_kits', 'invoicing_items_temp.item_kit_id = item_kits.item_kit_id', 'left');
	...
	public function getSummaryData()
	{
		if ($this->params['matched_items_only'])
		{
			$this->db->select('invoicing_items_temp.sale_id, sum(subtotal) as subtotal, sum(total) as total, sum(tax) as tax, sum(profit) as profit,sum(quantity_purchased) as items_purchased', false);
			$this->db->from('invoicing_items_temp');
			$this->db->where('invoicing_items_temp.deleted', 0);
			$this->_searchSalesQueryParams();
			$this->db->group_by('sale_id');
