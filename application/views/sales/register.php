						...
						<div class="col-md-8 no-padd">
							<div class="input-append">
								...
								<?php echo anchor("sales/suspended/",
									...
									array('class'=>'btn btn-danger none suspended_sales_btn','title'=>lang('sales_suspended_sales')));
									?>
								</form>
							</div>
						</div>					
				...
				<table id="register" class="table table-bordered">
					...
					<thead>
						<tr>
							...
							<!-- adding cost price -->
							<th class="sales_price"><?php echo 'Cost price'; ?></th>
							<th class="sales_price"><?php echo 'Whole Sale price'; ?></th>
							<!-- <th class="sales_price"><?php echo 'Retail price'; ?></th> -->
							...
							foreach(array_reverse($cart, true) as $line=>$item)	
							{
								...
								<tr id="reg_item_top" bgcolor="#eeeeee" >
									...
									<td class="text text-warning sales_stock" id="reg_item_stock" >
										<?php
											$query = $this->Item->get_cost_price($item['item_id']);
									...
									<td>
										...
										$query = $this->Item->get_wholesale_price($item['item_id']);
										foreach ($query->result() as $row)
										{
											echo $row->wholesale_price;
											//echo "<input type='hidden' name='wholesale' value='$row->wholesale_price'/>"; 
											//echo form_hidden('wholesale',$row->wholesale_price);
											$whole_out = $row->wholesale_price;
										}
										?>
									...
									<!-- <td>
										...
										$query = $this->Item->get_retail_price($item['item_id']);
										foreach ($query->result() as $row)
										{
											echo $row->retail_price;
											$retail_out = $row->retail_price;
										}
										?>
									...
									<td>
                    					...
                    					echo form_input(array('name'=>'price','value'=>to_currency_no_money($selling_price, 10),'class'=>'input-small', 'id' => 'price_'.$line));
										if ($this->Employee->has_module_action_permission('sales', 'edit_sale',  $this->Employee->get_logged_in_employee_info()->person_id)){
											echo form_hidden('permission',1);
										}
										echo form_hidden('wholesale',$whole_out);
										echo form_hidden('retailsale',$retail_out);
								 ...
								 ?>
									...
									<td class="text text-main"><?php 
									//editing price amount if admin
		...
		<ul class="list-inline pull-left">
			...
			<?php if ($mode != 'store_account_payment') { ?>
				...
				<?php
					...
					<li>
						<?php /* echo
						anchor("reports/invoicing_generator",
							lang('invoice_search_reports'),
							array('class'=>'btn btn-warning ',
								'title'=>lang('invoice_search_reports'))); */
						?>
					</li>
					<?php } ?>
			<?php } ?>
				...
				if ($this->Employee->has_module_action_permission('sales', 'give_discount', $this->Employee->get_logged_in_employee_info()->person_id) && $mode != 'store_account_payment'){ ?>
					<ul class="list-inline pull-right" id="global_discount">
						<li>
							...
							echo '%&nbsp;&nbsp;';
							echo form_submit('submit_discount_form',lang('common_submit'),'class="btn btn-success"');
							?>
							</form>
				...
				<li class="list-group-item">
					...
					<div id="finish_sale">
								...
								<div class="form-group form-group-sm">
									<div class="col-xs-8" style="display:none">
										<label class="control-label checkbox" for="sales_invoice_enable">
											...
											<input type="checkbox" name="custom7" id="custom7">
											<label for="custom7">Mark As Invoice</label><br>

										</label>
									</div>

									<div class="col-xs-8" style="display:none">
										<label class="control-label checkbox" for="sales_invoice_enable1">

											<input type="checkbox" name="custom8" id="custom8">
											<label for="custom8">Pay Invoice</label><br>
						...
						{
							echo "<input type='submit' class='btn btn-success btn-large btn-block' id='finish_sale_button' value='".lang('sales_complete_sale')."' />";
						}
						?>
					</div>
				</form>
...
<script type="text/javascript">
	...
	if (isset($warning2))
	{
		echo "gritter(".json_encode(lang('common_warning')).",".json_encode($warning2).",'gritter-item-warning',false,false);";
