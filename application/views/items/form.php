		...
		<div class="col-md-12">
			...
			<div class="widget-title widget-title1 pricing-widget">
				...
				<h5><?php echo lang("items_pricing_and_inventory"); ?></h5>
					...
					<?php if ($this->Employee->has_module_action_permission('items','see_cost_price', $this->Employee->get_logged_in_employee_info()->person_id) or $item_info->name=="") { ?>
						<div class="form-group">
							<?php echo form_label(('Whole sale price').' ('.lang('items_without_tax').')'.':', 'whole sale price',array('class'=>'col-sm-3 col-md-3 col-lg-2 control-label required wide')); ?>
							<div class="col-sm-9 col-md-9 col-lg-10">
								<?php echo form_input(array(
										...
										'id'=>'wholesale_price',
										'class'=>'form-control form-inps',
										'value'=>$item_info->wholesale_price ? to_currency_no_money($item_info->wholesale_price,10) : '')
								);?>
							</div>
						</div>
						<?php
					...
					{
						echo form_hidden('retail_price', $item_info->retail_price);
					}
					?>
					...
					<?php if ($this->Location->count_all() > 1) {?>
						...
						<div class="item-location-price-container <?php if ($location_items[$location->location_id] === FALSE || !$location_items[$location->location_id]->is_overwritten){echo 'hidden';} ?>">	
							...
							<div class="item-location-price-container <?php if ($location_items[$location->location_id] === FALSE || !$location_items[$location->location_id]->is_overwritten){echo 'hidden';} ?>">
								<?php if ($this->Employee->has_module_action_permission('items','see_cost_price', $this->Employee->get_logged_in_employee_info()->person_id) or $item_info->name=="") { ?>
									<div class="form-group">
										<?php echo form_label(('Whole sale price').' ('.lang('items_without_tax').'):', '',array('class'=>'col-sm-3 col-md-3 col-lg-2 control-label wide')); ?>
										<div class="col-sm-9 col-md-9 col-lg-10">
											<?php echo form_input(array(
													'name'=>'locations['.$location->location_id.'][wholesale_price]',
													'size'=>'8',
													'class'=>'form-control form-inps',
													'value'=> $location_items[$location->location_id]->item_id !== '' && $location_items[$location->location_id]->wholesale_price ? to_currency_no_money($location_items[$location->location_id]->wholesale_price, 10): ''
												)
											);?>
										</div>
									</div>
								...
								{
									echo form_hidden('locations['.$location->location_id.'][wholesale_price]', $location_items[$location->location_id]->item_id !== '' ? $location_items[$location->location_id]->wholesale_price: '');
								}
								?>
...
$(document).ready(function()
{
	...
	$('#item_form').validate({
		...
		<?php foreach($locations as $location) { ?>
			...
			"<?php echo 'locations['.$location->location_id.'][wholesale_price]'; ?>":
			{
				number: true
			},
			"<?php echo 'locations['.$location->location_id.'][retail_price]'; ?>":
			{
				number: true
			},
		...
		{			
			...
			wholesale_price:
			{
				required:<?php echo json_encode(('Items whole sale price required')); ?>,
				number:<?php echo json_encode(('Items whole sale price required')); ?>
			},
			retail_price:
			{
				required:<?php echo json_encode(('Items retail sale price required')); ?>,
				number:<?php echo json_encode(('Items retail sale price required')); ?>
			},
			unit_price:
			{
