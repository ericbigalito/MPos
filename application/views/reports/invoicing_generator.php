...
(function($) 
{
  	$.fn.tokenize = function(options)
	{
		...
		{
      		$(this).tokenInput('<?php echo site_url("reports/invoicing_generator"); ?>?act=autocomplete',
			{
				theme: "facebook",
				queryParam: "term",
				extraParam: "w",
...
$(document).on('change', ".selectField", function(){
	...
	else 
	{
		...
		if ($(this).val() == 2 || $(this).val() == 7 || $(this).val() == 10 || $(this).val() == 12 || $(this).val() == 13)
		{
...
$(function() {
	...
	$(".selectField").each(function(i) {
		if ($(this).val() == 0) {
			...
			if ($(this).val() != 2 && $(this).val() != 6 && $(this).val() != 7 && $(this).val() != 10 && $(this).val() != 12 && $(this).val() != 13) {
...
function disableConditions(elm, q) {
	...
	$("option", disabled).each(function() {
		...
		else if (elm.val() == 13 && $.inArray($(this).attr("value"), allowed4) != -1) {
			$(this).removeAttr("disabled");
		}else if (elm.val() == 6 && $.inArray($(this).attr("value"), allowed3) != -1) {
			$(this).removeAttr("disabled");
		} else if (elm.val() == 7 && $.inArray($(this).attr("value"), allowed2) != -1) {
			$(this).removeAttr("disabled");
		} else if (elm.val() != 6 && elm.val() != 7 && elm.val() != 10 && elm.val() != 13 && elm.val() != 11 && $.inArray($(this).attr("value"), allowed1) != -1) {
			$(this).removeAttr("disabled");
		} 
	});
...
<form name="salesReportGenerator" action="<?php echo site_url("reports/invoicing_generator"); ?>" method="get" class="form-horizontal form-horizontal-mobiles">
...
<table id="contents" class="table">
	...
	<tr>
		<td class="item_table">
			<?php echo form_label(lang('reports_invoicing_generator_show_only_matched_items'), 'matched_items_only'); ?>
			<?php
				...
				<tr class="duplicate">
					<td class="field">
						<select name="field[]" class="selectField ">
							...
							<option value="10" rel="invoiceID"<?php if($field[$k] == 10) echo " selected='selected'";?>><?php echo lang("reports_invoicing_generator_selectField_10") ?></option>
				...
				<tr class="duplicate">
					<td class="field">
						<select name="field[]" class="selectField span7">
							...
							<option value="10" rel="invoiceID"><?php echo lang("reports_invoicing_generator_selectField_10") ?></option>
