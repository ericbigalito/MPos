...
<div class="clear"></div>
    ...
    <!-- <div class="row">
		<div class="col-md-6 center" style="text-align: center;">					
			<ul class="stat-boxes">
				<?php foreach($summary_cash_subract as $name=>$value) { ?>
				<li class="popover-visits">
					<div class="left peity_bar_good"><h5><?php echo lang('reports_'.$name).' Cash'; ?></h5></div>
					<div class="right">
                        <strong><?php echo to_currency($value); ?></strong>
    ...
    <!-- getting total cash paid to subtract from store account to subtract  -->
    <?php foreach($summary_cash_subract as $name=>$value) { 
					...
					<table class="table table-bordered table-striped table-hover data-table tablesorter" id="sortable_table">
						...
						<tbody>
							...
							<tr>
								...
								<?php if ($cell['data'] == "Store Account") { ?>
									<td align="<?php echo $cell['align'];?>"><?php echo $cell['data']." Cummulative"; ?></td>
								<?php } else { ?>
							...
							<?php } ?>
							<tr>
							<td align="left">Paid Invoices</td>
							<td align="right"><?php echo "Ksh".$subtract_value.".00" ?></td>
							</tr>
						</tbody>
					</table>
					</div>
