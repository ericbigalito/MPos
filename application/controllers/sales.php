...
class Sales extends Secure_area
{
	...
	{		
		...
		elseif($this->sale_lib->is_valid_item_kit($item_id_or_number_or_item_kit_or_receipt))
		{
			...
			if($this->sale_lib->check_expiry($item_kit_id))
			{
				$data['warning2'] = lang('sales_date_expiry');
			}
		}
		...
		if($this->sale_lib->check_expiry($item_id_or_number_or_item_kit_or_receipt))
		{
			$data['warning2'] = lang('sales_date_expiry');
		}
		$this->_reload($data);
	...
	function edit_item($line)
	{
		...
		$retailsale = $this->input->post("retailsale");
		...
		if ($this->form_validation->run() != FALSE)
		{
			if((($price < $wholesale) && ($permission != 1))){
				$data['error']='Error Editing Item Price, you do not have sufficient permission ';
			}
			// || (($discount < $retailsale) && ($permission != 1))
			else{
		...
		if($this->sale_lib->is_kit_or_item($line) == 'item')
		{
			...
			if($this->sale_lib->check_expiry($this->sale_lib->get_item_id($line)))
			{
				$data['warning2'] = lang('sales_date_expiry');
			}
		}
		elseif($this->sale_lib->is_kit_or_item($line) == 'kit')
		{
			...
			if($this->sale_lib->check_expiry($this->sale_lib->get_kit_id($line)))
			{
				$data['warning2'] = lang('sales_date_expiry');
			}
		}
	...
	function complete()
	{
		...
		$invoice_check = $this->input->post('custom7');
		$invoice_check2 = $this->input->post('custom8');
		$data['subtotal']=$this->sale_lib->get_subtotal();
		$data['taxes']=$this->sale_lib->get_taxes();		
		$data['total']=$this->sale_lib->get_total();
		if($invoice_check == 1){
			$data['receipt_title']=lang('invoice_receipt');
		}
		else{
		...
		{
			if($invoice_check == 1){
				$sale_info = $this->Sale->get_info_invoicing($suspended_change_sale_id)->row_array();
		...
		if($invoice_check == 1){
			$sale_id_raw = $this->Sale->save($data['cart'], $customer_id,$employee_id,$data['comment'],$data['show_comment_on_receipt'],$data['payments'], $suspended_change_sale_id, 0,$data['ref_no'],$data['auth_code'], $data['change_sale_date'], $data['balance'], $data['store_account_payment'],$invoice_check, $invoice_check2);
		...
		if($invoice_check == 1){
			$this->load->view("sales/invoicing",$data);
		}
		else{
	...
	function receipt_invoicing($sale_id)
	{
		//Before changing the sale session data, we need to save our current state in case they were in the middle of a sale
		$this->sale_lib->save_current_sale_state();

		$data['is_sale'] = FALSE;
		$sale_info = $this->Sale->get_info_invoicing($sale_id)->row_array();
		$this->sale_lib->clear_all();
		$this->sale_lib->copy_entire_sale($sale_id, true);
		$data['cart']=$this->sale_lib->get_cart();
		$data['payments']=$this->sale_lib->get_payments();
		...
		$data['total']=$this->sale_lib->get_total($sale_id);
		$data['receipt_title']=lang('sales_receipt');
		$data['comment'] = $this->Sale->get_comment_invoicing($sale_id);
		$data['show_comment_on_receipt'] = $this->Sale->get_comment_on_receipt_invoicing($sale_id);
		$data['transaction_time']= date(get_date_format().' '.get_time_format(), strtotime($sale_info['sale_time']));
		$customer_id=$this->sale_lib->get_customer();
	...
	function edit_invoicing($sale_id)
	{
		$data = array();
		...
		$data['sale_info'] = $this->Sale->get_info_invoicing($sale_id)->row_array();


		$this->load->view('sales/edit_invoicing', $data);
	}
	...
	function delete_invoicing($sale_id)
	{
		$this->check_action_permission('delete_sale');
		$data = array();

		if ($this->Sale->delete_invoicing($sale_id))
		{
			$data['success'] = true;
		}
		else
	...
	function undelete_invoicing($sale_id)
	{
		...
		if ($this->Sale->undelete_invoicing($sale_id))
		{
			$data['success'] = true;
		}
		else
	...
	function save_invoicing($sale_id)
	{
		$sale_data = array(
			'sale_time' => date('Y-m-d', strtotime($this->input->post('date'))),
			'customer_id' => $this->input->post('customer_id') ? $this->input->post('customer_id') : null,
		...
		$invoice_check = 1;
		if ($this->Sale->update_invoicing($sale_data, $sale_id, $invoice_check))
		{
			echo json_encode(array('success'=>true,'message'=>lang('sales_successfully_updated')));
		}
		else
	...
	function _reload1($data=array(), $is_ajax = true)
	{
		$person_info = $this->Employee->get_logged_in_employee_info();
		$modes = array('sale'=>lang('sales_sale'),'return'=>lang('sales_return'));
		...
		if ($is_ajax)
		{
			$this->load->view("sales/register_invoice",$data);
		}
		else
		{
			$this->load->view("sales/register_initial_invoicing",$data);
		}
	...
	function change_invoicing($sale_id,$invoice_check2)
	{
		$this->check_action_permission('edit_sale');
		$this->sale_lib->clear_all();
		$this->sale_lib->copy_entire_invoicing($sale_id);
		$this->sale_lib->set_change_sale_id($sale_id);
