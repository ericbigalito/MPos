	...
	*/
			...
			if ($this->input->post('locations'))
			{
				...
				{		        
					...
					$data = array(
						...
						'wholesale_price' => $override_prices && $item_location_data['wholesale_price'] != '' ? $item_location_data['wholesale_price'] : NULL,
					...
					if ($item_location_data['quantity'] != '' && !$this->input->post('is_service'))
					{
						$inv_data = array
							...
							'quantity_left'=>$item_location_data['quantity'] - $item_location_before_save->quantity,
							'trans_running'=>$item_location_data['quantity'],
							'location_id' => $location_id,
						);
	...
	function save_inventory($item_id=-1)
	{
		...
		(
			...
			'quantity_left'=>$this->input->post('newquantity'),
		...
		else
		{
			if (($handle = fopen($_FILES['file_path']['tmp_name'], "r")) !== FALSE)
			{
				...
				while (($data = fgetcsv($handle)) !== FALSE)
				{
					...
					'wholesale_price'	=>	$data[5]!=null ? $data[5] : 0,
					...
					{	
						...
						if (!$item_data['is_service'])
						{
							...
							(
								...
								'quantity_left'=>$data[9+$price_tiers_count] - $item_location_before_save->quantity,
								'trans_running'=>$data[9+$price_tiers_count],
								'location_id'=>$this->Employee->get_logged_in_employee_current_location_id()
							);
							$this->Inventory->insert($inv_data);
						}
