...
{	
	...
	// Invoice Generator Reports
	function invoicing_generator() {
		$this->check_action_permission('view_sales_generator');
		...
		if ($this->input->get('generate_report')) { // Generate Custom Raport
			...
			$params['tables'] = array(
				1 => 'invoicing_items_temp.customer_id', // Customers
				2 => 'invoicing_items_temp.serialnumber', // Item Sale Serial number
				3 => 'invoicing_items_temp.employee_id', // Employees
				4 => 'invoicing_items_temp.category', // Item Category
				5 => 'invoicing_items_temp.supplier_id', // Suppliers
				6 => '', // Sale Type
				7 => '', // Sale Amount
				8 => 'invoicing_items_temp.item_kit_id', // Item Kit Name
				9 => 'invoicing_items_temp.item_id', // Item Name
				10 => 'invoicing_items_temp.sale_id', // Sale ID
				11 => 'invoicing_items_temp.payment_type', // Payment Type
				12 => 'invoicing_items_temp.description', // Item Sale Serial number
			);
			$params['values'] = $tmpData;
			$params['offset'] = $this->input->get('per_page')  ? $this->input->get('per_page') : 0;
			$params['export_excel'] = $this->input->get('export_excel');

			$this->load->model('reports/Invoicing_generator');
			$model = $this->Invoicing_generator;
			$model->setParams($params);

			// Invoice Interval Reports
			$interval = array(
				'start_date' => $data['start_year'].'-'.$data['start_month'].'-'.$data['start_day'],
				'end_date' => $data['end_year'].'-'.$data['end_month'].'-'.$data['end_day']. ' 23:59:59'
			);
			if (!$this->Sale->create_invoicing_items_temp_table($interval))
			{
				$this->load->view('reports/rate_limit');
				return;
			}
			...
			{
				$summary_data[$key] = array(
					array('data'=>anchor('sales/receipt_invoicing/'.$row['sale_id'], '<i class="fa fa-print fa fa-2x"></i>', array('target' => '_blank')).' '.anchor('sales/edit_invoicing/'.$row['sale_id'], '<i class="fa fa-file-alt fa fa-2x"></i>', array('target' => '_blank')).' '.anchor('sales/edit_invoicing/'.$row['sale_id'], lang('common_edit').' '.$row['sale_id'], array('target' => '_blank')), 'align'=>'left'),
					array('data'=>date(get_date_format().'-'.get_time_format(), strtotime($row['sale_time'])), 'align'=>'left'),
					array('data'=>to_quantity($row['items_purchased']), 'align'=>'left'),
					array('data'=>$row['employee_name'], 'align'=>'left'),
					array('data'=>$row['customer_name'], 'align'=>'left'),
			...
			if (!$this->input->get('export_excel'))
			{
				$data['results'] = $this->load->view("reports/invoicing_generator_tabular_details", $reportdata, true);
			}
		}

		if (!$this->input->get('export_excel'))
		{
			$this->load->view("reports/invoicing_generator",$data);
		}
		else //Excel export use regular tabular_details
		{
			$this->load->view("reports/tabular_details",$reportdata);
	...
	function summary_invoicing($start_date, $end_date, $sale_type, $export_excel=0, $offset=0)
	{
		$this->check_action_permission('view_sales');
		$start_date=rawurldecode($start_date);
		$end_date=rawurldecode($end_date);

		$this->load->model('reports/Summary_invoicing');
		$model = $this->Summary_invoicing;
		$model->setParams(array('start_date'=>$start_date, 'end_date'=>$end_date, 'sale_type' => $sale_type, 'offset' => $offset, 'export_excel' => $export_excel));

		if (!$this->Sale->create_invoicing_items_temp_table(array('start_date'=>$start_date, 'end_date'=>$end_date, 'sale_type' => $sale_type)))
		{
			$this->load->view('reports/rate_limit');
			return;
		}
		...
		$config = array();
		$config['base_url'] = site_url("reports/summary_invoicing/".rawurlencode($start_date).'/'.rawurlencode($end_date)."/$sale_type/$export_excel");
		$config['total_rows'] = $model->getTotalRows();
		$config['per_page'] = $this->config->item('number_of_items_per_page') ? (int)$this->config->item('number_of_items_per_page') : 20;
		$config['uri_segment'] = 7;
		$this->pagination->initialize($config);
		...
		$data = array(
			"title" => lang('reports_invoicing_summary_report'),
			"subtitle" => date(get_date_format(), strtotime($start_date)) .'-'.date(get_date_format(), strtotime($end_date)),
			"headers" => $model->getDataColumns(),
			"data" => $tabular_data,
			"summary_data" => $model->getSummaryData(),
	...
	function summary_payments($start_date, $end_date, $sale_type, $export_excel=0, $offset=0)
	{
		...
		$data = array(
			...
			"data" => $tabular_data,
			"summary_data" => $model->getSummaryData(),
			"summary_cash_subract" => $model->getSummaryDataSub(),
			"export_excel" => $export_excel,
			"pagination" => $this->pagination->create_links(),
		);
	...
	//detailed invoice
	function detailed_invoicing($start_date, $end_date, $sale_type, $export_excel=0, $offset = 0)
	{
		$this->check_action_permission('view_sales');
		$start_date=rawurldecode($start_date);
		$end_date=rawurldecode($end_date);

		$this->load->model('reports/Detailed_invoicing');
		$model = $this->Detailed_invoicing;
		$model->setParams(array('start_date'=>$start_date, 'end_date'=>$end_date, 'sale_type' => $sale_type, 'offset' => $offset, 'export_excel' => $export_excel));
		...
		$config = array();
		$config['base_url'] = site_url("reports/detailed_invoicing/".rawurlencode($start_date).'/'.rawurlencode($end_date)."/$sale_type/$export_excel");
		$config['total_rows'] = $model->getTotalRows();
		$config['per_page'] = $this->config->item('number_of_items_per_page') ? (int)$this->config->item('number_of_items_per_page') : 20;
		$config['uri_segment'] = 7;
		$this->pagination->initialize($config);
		...
		$data = array(
			"title" =>lang('reports_detailed_invoicing_report'),
			"subtitle" => date(get_date_format(), strtotime($start_date)) .'-'.date(get_date_format(), strtotime($end_date)),
			"headers" => $model->getDataColumns(),
			"summary_data" => $summary_data,
			"details_data" => $details_data,
	...
	function inventory_summary($supplier = -1, $export_excel=0, $offset = 0)
	{
		...
		{
			//$tabular_data[] = array(array('data'=>$row['name'], 'align'=> 'left'), array('data'=>$row['category'], 'align'=> 'left'), array('data'=>$row['company_name'], 'align'=> 'left'), array('data'=>$row['item_number'], 'align'=> 'left'), array('data'=>$row['product_id'], 'align'=> 'left'), array('data'=>$row['description'], 'align'=> 'left'), array('data'=>to_currency($row['cost_price']), 'align'=> 'right'),array('data'=>to_currency($row['unit_price']), 'align' => 'right'), array('data'=>to_quantity($row['quantity']), 'align'=> 'left'), array('data'=>to_quantity($row['reorder_level']), 'align'=> 'left'));
			$invested = ($row['cost_price']) *  ($row['quantity']);
			$value = ($row['unit_price']) * ($row['quantity']);
			$tabular_data[] = array(array('data'=>$row['name'], 'align'=> 'left'), array('data'=>$row['category'], 'align'=> 'left'), array('data'=>$row['company_name'], 'align'=> 'left'), array('data'=>to_currency($row['cost_price']), 'align'=> 'right'),array('data'=>to_currency($row['unit_price']), 'align' => 'right'), array('data'=>to_quantity($row['quantity']), 'align'=> 'left'), array('data'=>to_currency($invested), 'align'=> 'left'), array('data'=>to_currency($value), 'align'=> 'left'), array('data'=>to_quantity($row['reorder_level']), 'align'=> 'left'));
	...
	function summary_store_accounts($export_excel = 0, $offset=0)
	{
		...
		$data = array(
			...
			"data" => $tabular_data,
			//"summary_data" => $model->getSummaryData(array()),
			"summary_data" => $model->getTotalBal(),
			"export_excel" => $export_excel,
			'pagination' => $this->pagination->create_links()
		);
